...
class OAuth extends RequestHandler {
	 ...
	 * @arg {Object} options
	 * @arg {String} options.botToken The token of the bot used to authenticate
	 * @arg {String} options.guildId The ID of guild where member is
	 * @arg {String} options.userId The ID of the user to to get a role
	 * @arg {String} options.roleId The role id to assign to member
	 */
	addMemberRole(options) {
		return this.request(
			"PUT",
			`/guilds/${options.guildId}/members/${options.userId}/roles/${options.roleId}`,
		);
	}
